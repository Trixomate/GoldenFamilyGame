...
export const QuestionBoard3: React.FC<QuestionBoardProps> = ({ item, revealed, onReveal, boardNumber, totalBoards }) => (
  <div className="absolute inset-0 flex w-full">
    ...
    {/* Right: Answer Cards (3 Large Cards) */}
    <div className="w-1/2 p-[5%] flex flex-col justify-center space-y-[2.5vh] bg-black/10 overflow-hidden">
      {item.answers?.map((answer, idx) => (
        <AnswerCard 
          ...
          heightClass="h-[15vh]" // Default height for 3 items
        />
      ))}
    </div>
  </div>
