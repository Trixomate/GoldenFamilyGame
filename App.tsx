...
// --- Types ---
interface Answer { text: string; percentage: number; }
interface Question { id: number; question: string; answers: Answer[]; }

// --- Sub-components ---

// 1. Team Control Panel (Footer)
const TeamControl: React.FC<{
  name: string;
  score: number;
  setScore: (fn: (n: number) => number) => void;
  strikes: boolean[];
  toggleStrike: (idx: number) => void;
  reverse?: boolean;
}> = ({ name, score, setScore, strikes, toggleStrike, reverse = false }) => {
  const StrikesDisplay = () => (
    <div className="flex space-x-[0.5vw]">
      {strikes.map((active, i) => (
        <button key={i} onClick={() => toggleStrike(i)} className={`w-[5vh] h-[5vh] rounded-full border-[0.3vh] transition-all duration-300 flex items-center justify-center shadow-xl ${active ? 'bg-red-600/95 border-red-400 shadow-red-900/50 scale-105 ring-[0.5vh] ring-red-500/20' : 'bg-zinc-800/60 border-zinc-700/40 grayscale opacity-40 hover:grayscale-0 hover:opacity-100'}`}>
...
const AnswerCard: React.FC<{
  ...
  return (
    <div className="relative h-[15vh] perspective-1000 group shrink-0">
      <button onClick={onReveal} className={`absolute inset-0 w-full h-full flex items-center justify-between px-[3vw] rounded-[1.5vh] border-[0.3vh] transition-all duration-700 transform-gpu cursor-pointer shadow-xl shadow-black/60 bg-gradient-to-b ${s.gradient} ${s.border} ${s.shadow} ${revealed ? 'rotate-x-180 opacity-0 pointer-events-none scale-90' : 'hover:-translate-y-1 active:scale-95'}`}>
        <div className="flex items-center space-x-[2vw]">
          <span className={`text-[5vh] font-black italic transition-all drop-shadow-md ${s.numText} group-hover:text-white`}>{index + 1}</span>
      ...
      <div className={`absolute inset-0 w-full h-full flex items-center justify-between bg-gradient-to-r ${s.revealedGradient} rounded-[1.5vh] border-[0.5vh] ${s.revealedBorder} shadow-2xl transition-all duration-700 transform-gpu backface-hidden overflow-hidden ${revealed ? 'rotate-x-0 opacity-100 scale-100' : 'rotate-x-[-110deg] opacity-0 scale-75'}`}>
        ...
        <span className={`relative z-10 pl-[3vw] text-[4vh] font-black ${s.textColor} uppercase tracking-tighter drop-shadow-sm`}>{answer.text}</span>
      ...
      >
        ...
        {currentIndex >= 0 && (
          <div className="h-[15%] shrink-0 bg-black/60 border-t border-white/10 backdrop-blur-md flex items-center justify-between z-30 relative overflow-hidden">
            <TeamControl name="Famille A" score={scoreA} setScore={setScoreA} strikes={strikesA} toggleStrike={toggleStrikeA} />
            <div className="w-[1px] h-[60%] bg-gradient-to-b from-transparent via-white/20 to-transparent"></div>
            <TeamControl name="Famille B" score={scoreB} setScore={setScoreB} strikes={strikesB} toggleStrike={toggleStrikeB} reverse={true} />
          </div>
        )}
